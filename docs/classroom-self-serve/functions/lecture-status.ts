// lecture-status v2 — AUTHORIZED poll endpoint for the New Lecture build.
//
// Project: Shryn website (cmoamdistlpbahcryjda). Mirror of the deployed edge function.
//
// v2: this is a professor tool, so it now requires the subject owner's JWT (or
// the service-role key). Previously anyone with a lecture UUID could read an
// unpublished/private lecture's video_url + teaching_mode_prompt.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: l, error } = await sb.from("classroom_lectures")
      .select("id, subject_id, subject_slug, lecture_slug, lecture_title, status, error_message, video_url, captions_url, teaching_mode_prompt, duration_seconds, is_public, access_mode")
      .eq("id", lectureId).single();
    if (error || !l) return json({ error: "Lecture not found" }, 404);

    // Non-service callers may only see their own subjects' lectures.
    if (!auth.isService) {
      const { data: owned } = await sb.from("subject_profiles")
        .select("id").eq("id", l.subject_id).eq("user_id", auth.userId).maybeSingle();
