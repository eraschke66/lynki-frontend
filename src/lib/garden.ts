...
export function getGardenStatus(percentage: number): GardenStatus {
  if (percentage >= 75) return { label: "In full bloom", japanese: "満開", description: "Your garden is thriving.", color: "text-emerald-700", bgColor: "bg-emerald-50" };
  if (percentage >= 60) return { label: "Blooming", japanese: "開花", description: "Things are coming together beautifully.", color: "text-teal-700", bgColor: "bg-teal-50" };
  if (percentage >= 40) return { label: "Finding its rhythm", japanese: "成長", description: "Steady growth. The roots are strong.", color: "text-green-700", bgColor: "bg-green-50" };
  if (percentage >= 20) return { label: "Taking root", japanese: "芽生え", description: "Every garden starts here.", color: "text-lime-700", bgColor: "bg-lime-50" };
  return { label: "Freshly planted", japanese: "種", description: "The seed is in the ground. Let's tend it.", color: "text-amber-700", bgColor: "bg-amber-50" };
}
...
export function getAnswerFeedback(correct: boolean): { title: string } {
  return correct ? { title: "That one took root." } : { title: "That seed needs more light." };
}
...
export function getProgressMessage(current: number, previous: number | null): string {
  if (previous === null) return current >= 40 ? "A good start. The roots are forming." : "Every garden begins with a single seed.";
